...
if( (!isNull cursorTarget) && (player distance cursorTarget < 5) && (vehicle player == player) ) then {
    ...
    _type = switch true do {
        ...
        case ( (_cTarget isKindOf "Man") && (alive _cTarget) && (!isNull unitBackpack _cTarget) && (_cTarget getVariable ["NORRN_unconscious", false]) ):{"open_backpack"}; //Person with backpack
